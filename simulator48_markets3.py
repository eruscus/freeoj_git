...
choice_indices = numpy.arange(len(products) * NUM_MARKETS * NUM_WEEKS)
choice_columns = ['Product', 'Region', 'Market', 'Demand_Week', 'Storage_Week', 'Plant_Week', 'Purchase_Week', 'Grove', 'Plant', 'Storage', 'Transporter', 'Price', 'Path_Cost','Quantity', 'Profit', 'Year_Used', 'Contract']
    ...
    for p in products:
        for r in regions:
            for w in weeks:  
                ...
                market_parameters = get_market_parameters(p, r, w, slope, intercept, 'SPOT_ORA')
                num_markets = len(market_parameters)
                for x in range(0, num_markets):
                    choices_SPOT_ORA.iloc[i*num_markets+x:i*num_markets+x+1, 0:17] = market_parameters[x]
                
                market_parameters = get_market_parameters(p, r, w, slope, intercept, 'FUT_ORA')
                num_markets = len(market_parameters)
                for x in range(0, num_markets):
                    choices_FUT_ORA.iloc[i*num_markets+x:i*num_markets+x+1, 0:17] = market_parameters[x]
                
                if (p == 'ROJ') | (p == 'FCOJ'):
                    market_parameters = get_market_parameters(p, r, w, slope, intercept, 'FUT_FCOJ')
                    num_markets = len(market_parameters)
                    for x in range(0, num_markets):
                        choices_FUT_FCOJ.iloc[i*num_markets+x:i*num_markets+x+1, 0:17] = market_parameters[x]
...
def get_market_parameters(product, region, demand_week, slope, intercept, contract):
    ...
    markets = get_markets(region)
    num_markets = len(markets)
    optimal_paths = numpy.empty(num_markets, dtype=numpy.ndarray)
    for i in range(0,num_markets):
        optimal_paths[i] = get_optimal_path(product, markets[i], purchase_month, contract) # month only needed for grove purchasing
        
    average_cost = 0.0
    for i in range(0,num_markets):
        single_cost = optimal_paths[i][4]
        average_cost = average_cost + single_cost
    average_cost = average_cost / num_markets
        
    # solve derivative of quadratic for quantity/price
    quantity = (average_cost - intercept) / (2 * slope)
    ...
    profit = quantity * (price - average_cost)
    ...
    for i in range(1,15):
        ...
        profit2 = quantity2 * (price2 - average_cost)
    ...
    market_parameters = numpy.empty(num_markets, dtype=numpy.ndarray)
    for i in range(0,num_markets):
        ...
        market_parameters[i] = [product, region, markets[i], demand_week, 
                          storage_week, plant_week, purchase_week, grove, 
                          plant, storage, transporter, price, average_cost, 
                          quantity/num_markets, profit/num_markets, year_used, 
                          contract]

    return market_parameters
    
# return cheapest path from purchase to delivery
def get_optimal_path(product, market, month, contract):
