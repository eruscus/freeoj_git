...
def get_optimal_parameters(product, region, demand_week, slope, intercept, contract):
    ...
    for i in range(0, num_markets):
        ...
        grove = grove + " " + str(optimal_path[0])
        plant = plant + " " + str(optimal_path[1])
        storage = storage + " " + str(optimal_path[2])
        transporter = transporter + "  " + str(optimal_path[3])
    ...
    parameters = [product, region, demand_week, storage_week, plant_week, 
                  purchase_week, grove, plant, storage, 
                  transporter, price, average_cost, quantity, profit, 
                  year_used, contract]
