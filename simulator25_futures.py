...
ORA_fut_prices.set_index([['2015','2016','2017','2018','2019','2020']], inplace=True)

FCOJ_fut_prices = raw_materials.iloc[33:39, 13:14]
FCOJ_fut_prices.columns = ['Price']
FCOJ_fut_prices.set_index([['2015','2016','2017','2018','2019','2020']], inplace=True)
...
def get_optimal_parameters(product, region, month, slope, intercept):
    ...
    best_grove = get_best_grove(product, region, month, closest_plant, closest_storage, transporter)
...
def get_best_grove(product, region, month, closest_plant, closest_storage, transporter):
    min_cost = float('inf')
    for g in groves:
        d_cost = get_transportation_cost(product, region, grove, closest_plant, closest_storage, transporter)
        g_cost = get_purchase_cost(product, g, month)
...
def get_purchase_cost(product, grove, month):
    if (g in ['FLA','CAL','TEX','ARZ','BRA','SPA']):
