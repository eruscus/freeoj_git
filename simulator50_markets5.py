...
max_capacities = [1748, 4016, 8436, 889]
NUM_WEEKS = 48
...
ORA_equations = pandas.read_csv('Data/ORA_changed_points.csv')
POJ_equations = pandas.read_csv('Data/POJ_changed_points.csv')
ROJ_equations = pandas.read_csv('Data/ROJ_changed_points.csv')
FCOJ_equations = pandas.read_csv('Data/FCOJ_changed_points.csv')
...
def get_optimal_parameters(product, region, demand_month, path_cost, avg_cost, slope, intercept):
    ...
    # log-linear model, q = a*b^p
#    if (slope != 'inf') & (intercept != 0):
##        print(product)
##        print(region)
##        print(demand_month)
##        print(slope)
##        print(intercept)
#        price = (avg_cost - 1/math.log(intercept))
#        quantity = slope*math.pow(intercept, price)
#    else:
#        price = 0
#        quantity = 0
...
#product = paths_SPOT_ORA.loc[0, 'Product']
#region = paths_SPOT_ORA.loc[0, 'Region']
#demand_week = paths_SPOT_ORA.loc[0, 'Demand_Week']
#avg_cost = paths_SPOT_ORA.loc[0, 'Avg_Cost']
#
#demand_month = months[int(math.floor(demand_week / 4.0))] # convert from week to month
#
#
