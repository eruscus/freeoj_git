...
def get_optimal_parameters(product, region, demand_week, slope, intercept, contract):
    ...
    # remember individual market paths
    markets = get_markets(region)
    num_markets = len(markets)
    
    average_cost = 0.0
    grove_array = ["" for x in range(num_markets)]
    plant_array = ["" for x in range(num_markets)]
    storage_array = ["" for x in range(num_markets)]
    transporter_array = ["" for x in range(num_markets)]
    for i in range(0, num_markets):
        optimal_path = get_optimal_path(product, markets[i], purchase_month, contract) # month only needed for grove purchasing
        grove_array[i] = optimal_path[0]
        plant_array[i] = optimal_path[1]
        storage_array[i] = optimal_path[2]
        transporter_array[i] = optimal_path[3]
        average_cost = average_cost + optimal_path[4]
    ...
    parameters = [product, region, demand_week, storage_week, plant_week, 
                  purchase_week, grove_array, plant_array, storage_array, 
                  transporter_array, price, average_cost, quantity, profit, 
                  year_used, contract]
